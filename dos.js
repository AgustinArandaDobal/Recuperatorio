function mostrar() {
  //creo las variables y las inicializo si corresponde
  let nombreAlumno;
  let apellidoAlumno;
  let materiaCursada;
  ...
  do {
    ...
    do {
      ...
      materiaCursada != "programacion" &&
      materiaCursada != "matematica" &&
      materiaCursada != "ingles");

    do {
      notaPrimerCuatri = parseInt(prompt("Ingrese la nota del primer cuatrimestre. 0 a 10"));
    } while (
      isNaN(notaPrimerCuatri) ||
      notaPrimerCuatri < 0 ||
      notaPrimerCuatri > 10);
  ...
  alert(
    "El promedio de notas del primer cuatrimestr es: " +
      promedioPrimerCuatri +
      " y el promedio del segundo cuatrimestre es: " +
      promedioSegundoCuatri
  );

  //punto B
  alert("El alumno con menor nota del primer cuatrimestre se llama " + nombreMenorNotaPrimerCuatri +
      " " +
      apellidoMenorNotaPrimerCuatri +
      " y es de la comision " +
      comisionMenorNotaPrimerCuatri);

  //punto C
  if (contadorNotasProgramacion > contadorNotasIngles &&
    contadorNotasProgramacion > contadorNotasMatematica) {
